...
const register = (name, email, password, prefered) => async (dispatch) => {
  dispatch({ type: USER_REGISTER_REQUEST, payload: { name, email, password, prefered } });
  try {
    const { data } = await Axios.post("/api/users/register", { name, email, password, prefered });
